...
HttpServerCodec()  HTTP协议的encode和decode的结合

会解析出HttpRequest 和HttpContent两个内容。



SimpleChannelInboundHandler 可以只关心指定内容的消息，new SimpleChannelInboundHandler<HttpRequest>在泛型中加入HttpRequest，就只处理这个类型的消息。
